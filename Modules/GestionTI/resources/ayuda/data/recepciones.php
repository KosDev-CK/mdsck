...
return [
    ...
    'concepto' => 'La Recepción de Proveedor registra la llegada física de la mercancía de una Solicitud a Proveedores ya existente. Es la pantalla donde el inventario se vuelve real: por cada unidad de una línea marcada como "activo inventariable" que se reciba, el sistema da de alta un Activo nuevo con su propio folio de inventario, listo para asignarse después a un empleado. Una recepción, una vez guardada, no se puede editar ni cancelar — cantidades, folio, fechas y líneas son definitivos; si falta capturar algo más de la misma solicitud (un embarque parcial), se registra como una recepción nueva. La única excepción es la remisión digitalizada: puede adjuntarse después o corregirse si se vinculó el archivo equivocado (ver "Remisión digitalizada" más abajo).',
    ...
    'campos' => [
        ...
        ['nombre' => 'Remisión digitalizada', 'explicacion' => 'El comprobante de entrega del proveedor, ya sea subiendo un archivo nuevo o eligiendo uno ya existente en SharePoint (la lista excluye los archivos ya vinculados a otra recepción). Opcional al crear — si no se adjunta en ese momento, el botón "Adjuntar remisión" aparece después en el listado para agregarla; una vez que existe, el listado muestra "Ver remisión" y "Quitar" (corrige un archivo equivocado sin borrar el original de SharePoint/disco, y vuelve a habilitar "Adjuntar remisión").'],
