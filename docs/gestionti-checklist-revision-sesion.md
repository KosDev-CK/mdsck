...
- [x] **Commiteado, pusheado y desplegado (2026-09-04).** Commits `c0a9a98` (sistema de Ayuda completo, 60 archivos) y `c95b936` (documentación de cierre), ambos sobre `428e283`, pusheados a `origin/master`. `Modules/MesaServicio/` y sus 2 docs quedaron fuera del stage explícitamente (`git add` por rutas, nunca `-A`). Desplegado a producción con `deploy-mdsck.ps1 -Force` (`-Force` necesario solo por `MesaServicio` sin commitear en el working tree, no por cambios propios sin commitear) — deploy limpio, sin migraciones nuevas, `https://mds.ck.com.mx/login` responde 200 tras el deploy.
